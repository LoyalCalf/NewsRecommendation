...
"""根据数据库中的相似用户表进行推荐"""
class UserCF_Recommendation(object):
    ...
    def get_data(self,user_id):
        ...
        if user.exists():
            ...
            news_list = user_behavior.objects.filter(user_id=user_id,behavior_type__in=[1,2],behavior_time__range=[before_today,datetime.now()]).values_list('news_id',flat=True)
            #当前用户浏览资讯集合
            ...
            for similary_user in similary_user_list:
                ...
                #取出相似用户浏览过的资讯
                news_list = user_behavior.objects.filter(user_id=int(similary_user),behavior_type__in=[1,2],behavior_time__range=[before_today,datetime.now()]).values_list('news_id',flat=True)
